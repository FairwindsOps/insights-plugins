...
func (p *PolicySyncProcessor) executeSyncActions(ctx context.Context, actions PolicySyncActions, managedPoliciesByInsights []ClusterPolicy, result *PolicySyncResult) error {
	...
	for _, policyName := range actions.ToApply {
		if policy, exists := managedPoliciesByInsightsMap[policyName]; exists {
			if err := policyManager.applyPolicy(ctx, policy, p.config.DryRun); err != nil {
				...
				if !p.config.DryRun {
					err := p.insightsClient.UpdateKyvernoPolicyStatus(p.config.Token, policyName, "failed", string(policy.YAML), output)
					if err != nil {
				...
				if !p.config.DryRun {
					err := p.insightsClient.UpdateKyvernoPolicyStatus(p.config.Token, policyName, "success", string(policy.YAML), "")
					if err != nil {
						slog.Error("Failed to update policy status in Insights", "error", err)
					}
				}
	...
	for _, policyName := range actions.ToRemove {
		if err := policyManager.removePolicy(ctx, policyName, p.config.DryRun); err != nil {
			...
			output := strings.Join(result.Errors, "\n")
			if !p.config.DryRun {
				err := p.insightsClient.UpdateKyvernoPolicyStatus(p.config.Token, policyName, "failed", "Failed to remove policy from cluster", output)
				if err != nil {
					slog.Error("Failed to update policy status in Insights", "error", err)
				}
			}
			...
			if !p.config.DryRun {
				err := p.insightsClient.UpdateKyvernoPolicyStatus(p.config.Token, policyName, "success", "Successfully removed policy from cluster", "")
				if err != nil {
					slog.Error("Failed to update policy status in Insights", "error", err)
				}
			}
