...
func (ci *CIScan) GetTrivyReport(manifestImages []trivymodels.Image) (models.ReportInfo, error) {
	...
	trivyResults, trivyVersion, err := scanImagesWithTrivy(ci.config.Options.TempFolder, allImages)
	if err != nil {
		return trivyReport, err
	}
...
func scanImagesWithTrivy(tempFolder string, images []trivymodels.Image) ([]byte, string, error) {
	...
	for _, currentImage := range images {
		...
		logrus.Infof("Scanning %s with pullRef %s", currentImage.Name, currentImage.PullRef)
		results, err := image.ScanImage(tempFolder, "", currentImage.PullRef)
		if err != nil {
			return nil, "", err
		}
