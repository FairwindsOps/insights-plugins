...
if [[ "$provider" == "gcp" ]]; then
  ...
  touch /output/cloudcosts-tmp.json && more /output/cloudcosts-tmp.json && /google-cloud-sdk/bin/bq --format=prettyjson query --max_rows=10000000 --nouse_legacy_sql "$sql" > /output/cloudcosts-tmp.json
