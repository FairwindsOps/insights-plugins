...
if [[ "$provider" == "gcp" ]]; then
  ...
  r = /google-cloud-sdk/bin/bq --format=prettyjson --apilog=/output/logs.txt --debug_mode=true --httplib2_debuglevel=1 query --max_rows=10000000 --nouse_legacy_sql "$sql" > /output/cloudcosts-tmp.json
  echo "Google bigquey finished......"

  if [[ "$r" != "" ]]; then
    echo "Error running bigquery"
    cat /output/logs.txt
    echo "Error: $r"
    exit 1
  fi
