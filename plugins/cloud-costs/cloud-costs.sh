...
if [[ "$provider" == "gcp" ]]; then
  ...
  if [[ "$format" == "focus" ]]; then
    ...
    sql="SELECT * FROM \`$focusview\` WHERE ChargePeriodStart >= '$initial_date_time' AND ChargePeriodStart < '$final_date_time' AND (EXISTS (SELECT 1 FROM UNNEST(Tags) AS t WHERE t.key = '$tagkey' AND t.value = '$tagvalue') OR ResourceId LIKE '%/clusters/$tagvalue%') ORDER BY ChargePeriodStart DESC"
  else
