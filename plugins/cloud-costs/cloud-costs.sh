...
if [[ "$provider" == "gcp" ]]; then
  ...
  sql="SELECT cost, 0 AS cost_at_list, '' AS cost_type, service, sku, usage, usage_start_time, usage_end_time, CASE WHEN machine_spec IS NOT NULL THEN [ STRUCT('compute.googleapis.com/machine_spec' AS key, machine_spec AS value), STRUCT('compute.googleapis.com/cores' AS key, CAST(total_cores AS STRING) AS value), STRUCT('compute.googleapis.com/memory' AS key, CAST(total_memory AS STRING) AS value)  ] ELSE NULL END AS system_labels FROM (  SELECT SUM(main.cost) AS cost, STRUCT(main.service.description) AS service, STRUCT(main.sku.description AS description) AS sku, main.usage_start_time, main.usage_end_time, STRUCT(SUM(main.usage.amount) AS amount, SUM(main.usage.amount_in_pricing_units) AS amount_in_pricing_units, 0 AS pricing_unit, '' AS unit) AS usage, (SELECT value FROM UNNEST(system_labels) WHERE key = 'compute.googleapis.com/machine_spec') AS machine_spec, SUM(CAST((SELECT value FROM UNNEST(system_labels) WHERE key = 'compute.googleapis.com/cores') AS FLOAT64)) AS total_cores, SUM(CAST((SELECT value FROM UNNEST(system_labels) WHERE key = 'compute.googleapis.com/memory') AS FLOAT64)) AS total_memory FROM \`$table\` AS main LEFT JOIN UNNEST(main.system_labels) on true LEFT JOIN UNNEST(labels) AS labels WHERE labels.key = '$tagkey' AND labels.value = '$tagvalue' and usage_start_time >= '$initial_date_time' AND usage_start_time < '$final_date_time' AND TIMESTAMP_TRUNC(_PARTITIONTIME, DAY) >= '$initial_date_time' AND TIMESTAMP_TRUNC(_PARTITIONTIME, DAY) <= '$final_date_time' GROUP BY main.service.description, usage_start_time, usage_end_time, main.sku.description, machine_spec) order by usage_start_time desc"
