...
var opts struct {
	...
	Namespaces                    []string      `env:"RIGHTSIZER_NAMESPACES" short:"N" long:"namespace" env-delim:"," description:"Only respond to OOM-killed containers in these Kubernetes namespaces. By default, all namespaces are allowed. This applies both to alerting on OOM-kills and modifying memory limits if --update-memory-limits is enabled. Use this option multiple times to specify multiple namespaces. IF setting namespaces via the environment variable, separate namespaces by a comma."`
	UpdateNamespaces              []string      `env:"RIGHTSIZER_UPDATE_NAMESPACES" short:"n" long:"update-memory-limits-namespace" env-delim:"," description:"Only update memory limits of OOM-killed containers in these Kubernetes namespaces. By default, all namespaces specified by the --namespace option are updated. Use this option multiple times to specify multiple namespaces. IF setting namespaces via the environment variable, separate namespaces by a comma."`
	...
	UpdateMemoryLimits            bool          `env:"RIGHTSIZER_UPDATE_MEMORY_LIMITS" short:"m" long:"update-memory-limits" description:"Update the memory limits of resources, such as deployments, that own pods that have been OOM-killed. Updating stops once the threshold is reached defined by the --update-memory-limits-max option. To specify which namespaces are allowed to be updated, see the --namespaces and --update-namespaces options."`
	UpdateMemoryLimitsMinimumOOMs int64         `env:"RIGHTSIZER_UPDATE_MEMORY_LIMITS_MIN_OOMS" short:"o" long:"update-memory-limits-min-ooms" description:"The number of OOM-kills required before the memory limits will be updated. This option is only used if --update-memory-limits is enabled."`
...
func main() {
	...
	eventController := controller.NewController(stopChan,
		...
		controller.WithAllowedUpdateNamespaces(opts.UpdateNamespaces),
