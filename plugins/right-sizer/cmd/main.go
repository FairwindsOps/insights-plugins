...
var opts struct {
	...
	UpdateMemoryLimitsMultiplier float64 `env:"RIGHTSIZER_UPDATE_MEMORY_LIMITS_MULTIPLIER" short:"u" long:"update-memory-limits-multiplier" description:"The number to multiply current memory limits, when updating them in response to an OOM-kill. This is only used if --update-memory-limits is enabled."`
	MaxMemoryLimitsUpdateFactor  float64 `env:"RIGHTSIZER_MAX_MEMORY_LIMITS_UPDATE_FACTOR" short:"U" long:"max-memory-update-limits-factor" description:"The number to multiply by the starting memory limits, to calculate the maxim memory limits value that should not be exceeded when updating limits. This is only used if --update-memory-limits is enabled."`
}
